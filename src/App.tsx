...
import { NoteEditor } from './components/NoteEditorTipTap'; // Using TipTap-based editor
...
function App() {
  ...
  // Format command handler (fallback for contentEditable, TipTap uses custom events)
  const handleFormat = useCallback(
    (command: string, value?: string) => {
      // TipTap handles formatting via custom events dispatched from Toolbar
      // This is a fallback for direct calls or non-TipTap usage
  ...
  const handleInsertContent = useCallback(
    (content: string) => {
      ...
      // For TipTap editor, dispatch a custom event to insert content
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('insert-content', { 
